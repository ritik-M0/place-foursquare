...
export const searchPoiTool = createTool({
  ...
  inputSchema: z.object({
    ...
    limit: z.number().optional().default(10).describe('Maximum number of responses to return'),
    lat: z.number().optional().describe('Latitude for biasing results'),
    lon: z.number().optional().describe('Longitude for biasing results'),
    radius: z.number().optional().describe('Radius in meters for constraining results'),
  }),
  ...
  execute: async ({ context }) => {
    ...
    if (!apiKey) {
      throw new Error('TomTom API key not found. Please set the TOMTOM_API_KEY environment variable.');
    }

    const url = new URL(`${baseURL}/search/${tomtomApiVersion}/poiSearch/${query}.json`);
...
export const getPlaceByIdTool = createTool({
  id: 'get-place-by-id',
  description: 'Get detailed information about a place using its entity ID from the TomTom Search API',
  inputSchema: z.object({
    entityId: z.string().describe('A unique POI, Street, Geography, Point Address, Address Range, or Cross Street identifier'),
    language: z.string().optional().describe('Language for the results (e.g., en-US, es-ES)'),
    openingHours: z.boolean().optional().describe('Include opening hours information'),
    ...
    view: z.enum(['Unified', 'US']).optional().describe('View type for the response'),
  }),
  outputSchema: z.object({
    summary: z.object({
      query: z.string(),
  ...
  execute: async ({ context }) => {
    const { entityId, language, openingHours, timeZone, mapcodes, relatedPois, view } = context;
    const apiKey = process.env.TOMTOM_API_KEY;
    ...
    if (openingHours !== undefined) url.searchParams.append('openingHours', openingHours.toString());
    ...
    if (!response.ok) {
      ...
      throw new Error(`TomTom Place by ID API request failed with status ${response.status}: ${errorText}`);
    }
...
export const getPoiPhotosTool = createTool({
  id: 'get-poi-photos',
  description: 'Get photos for a specific Point of Interest from the TomTom API',
  inputSchema: z.object({
  ...
  execute: async ({ context }) => {
    ...
    if (!response.ok) {
      const errorText = await response.text();
      throw new Error(`TomTom POI Photos API request failed with status ${response.status}: ${errorText}`);
    }
