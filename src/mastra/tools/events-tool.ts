...
export const searchEventsTool = createTool({
  id: 'search-events',
  description: 'Search for events using the PredictHQ API. Find concerts, sports events, festivals, conferences, and more.',
  inputSchema: z.object({
    q: z.string().optional().describe('Keyword to search for (e.g., "concert", "festival", "NBA")'),
    ...
    'start.lte': z.string().optional().describe('Events starting on or before this date (ISO 8601: YYYY-MM-DD or YYYY-MM-DDTHH:MM:SSZ)'),
    'end.gte': z.string().optional().describe('Events ending on or after this date (ISO 8601: YYYY-MM-DD or YYYY-MM-DDTHH:MM:SSZ)'),
    'end.lte': z.string().optional().describe('Events ending on or before this date (ISO 8601: YYYY-MM-DD or YYYY-MM-DDTHH:MM:SSZ)'),
    'active.gte': z.string().optional().describe('Events active on or after this date'),
    'active.lte': z.string().optional().describe('Events active on or before this date'),
    'updated.gte': z.string().optional().describe('Events updated on or after this date'),
    'updated.lte': z.string().optional().describe('Events updated on or before this date'),
    state: z.enum(['active', 'deleted']).optional().default('active').describe('Event state filter'),
    sort: z.enum(SORT_OPTIONS).optional().default('start').describe('Sort order for results'),
    'rank.gte': z.number().min(0).max(100).optional().describe('Minimum rank threshold (0-100)'),
    'rank.lte': z.number().min(0).max(100).optional().describe('Maximum rank threshold (0-100)'),
    'local_rank.gte': z.number().min(0).max(100).optional().describe('Minimum local rank threshold (0-100)'),
    'local_rank.lte': z.number().min(0).max(100).optional().describe('Maximum local rank threshold (0-100)'),
    'aviation_rank.gte': z.number().min(0).max(100).optional().describe('Minimum aviation rank threshold (0-100)'),
    'aviation_rank.lte': z.number().min(0).max(100).optional().describe('Maximum aviation rank threshold (0-100)'),
    ...
    brand_safe: z.boolean().optional().describe('Filter for brand-safe events only'),
    deleted_reason: z.string().optional().describe('Reason for deletion (when state=deleted)'),
    duplicate_of_id: z.string().optional().describe('Original event ID for duplicates'),
  }),
  outputSchema: z.object({
    count: z.number().describe('Total number of events matching the query'),
    overflow: z.boolean().optional().describe('True if there are more results than can be returned'),
    next: z.string().optional().describe('URL for next page of results'),
    previous: z.string().optional().describe('URL for previous page of results'),
    results: z.array(
      z.object({
        ...
        local_rank: z.number().optional().describe('Local popularity rank (0-100)'),
        aviation_rank: z.number().optional().describe('Aviation impact rank (0-100)'),
        ...
        location: z.object({
          ...
          coordinates: z.array(z.number()).describe('Longitude and latitude coordinates'),
        ...
        relevance: z.number().optional().describe('Relevance score for search query'),
        state: z.enum(['active', 'deleted']).describe('Current state of the event'),
        brand_safe: z.boolean().optional().describe('Whether event is brand safe'),
        deleted_reason: z.string().optional().describe('Reason for deletion if applicable'),
        duplicate_of_id: z.string().optional().describe('ID of original event if duplicate'),
        entities: z.array(
          z.object({
            entity_id: z.string(),
            name: z.string(),
        ...
        ).optional().describe('Related entities (performers, teams, etc.)'),
      }),
    ),
  }),
  execute: async ({ context }) => {
    ...
    if (localRankGte !== undefined) params.append('local_rank.gte', localRankGte.toString());
    if (localRankLte !== undefined) params.append('local_rank.lte', localRankLte.toString());
    if (aviationRankGte !== undefined) params.append('aviation_rank.gte', aviationRankGte.toString());
    if (aviationRankLte !== undefined) params.append('aviation_rank.lte', aviationRankLte.toString());
    if (place_id) params.append('place.id', place_id);
    if (relevance) params.append('relevance', relevance);
    if (brand_safe !== undefined) params.append('brand_safe', brand_safe.toString());
    ...
    try {
      ...
      if (error instanceof Error) {
        throw new Error(`Failed to fetch events from PredictHQ: ${error.message}`);
      }
