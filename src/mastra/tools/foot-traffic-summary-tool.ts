...
export const getFootTrafficSummaryTool = createTool({
  ...
  description: 'Gets a summarized, high-level foot traffic forecast for a venue. Use this when analyzing many locations to avoid running out of memory.',
  ...
  outputSchema: z.object({
    ...
    overall_busyness: z.number().optional().describe('An overall busyness score from 0 (quiet) to 100 (very busy).'),
    peak_hours_summary: z.string().optional().describe('A human-readable summary of the busiest day and hours.'),
  }),
  execute: async (toolExecution) => {
    const detailedData = await getFootTrafficTool.execute(toolExecution) as FootTrafficResponse;
    ...
    if (dailyAverages.length > 0) {
      const totalAverage = dailyAverages.reduce((a: number, b: number) => a + b, 0) / dailyAverages.length;
